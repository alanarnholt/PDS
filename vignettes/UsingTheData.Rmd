...
Once the `PDS` package is installed, you will need to load the package using the `library` function.  If you need directions for installing the `PDS` package, see [https://github.com/alanarnholt/PDS](https://github.com/alanarnholt/PDS).   
...
Note that the variable `S3AQ3B1` renamed to `SmokingFrequency` uses a 9 to record `unknown` for smoking frequency.  To find a word/variable in a `pdf` use `shift F` with Windows or `command F` with OSX. Type the desired word/variable inside the box and press enter/return.
